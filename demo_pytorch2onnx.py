...
def transform_to_onnx(weight_file, batch_size, n_classes, IN_IMAGE_H, IN_IMAGE_W):
    ...
    if dynamic:
        x = torch.randn((1, 3, IN_IMAGE_H, IN_IMAGE_W), requires_grad=True)   # .cuda()
        onnx_file_name = "yolov4_-1_{}_{}_dynamic.onnx".format(IN_IMAGE_H, IN_IMAGE_W)
        ...
        onnx_file_name = "yolov4_{}_{}_{}_static.onnx".format(batch_size, IN_IMAGE_H, IN_IMAGE_W)
        # Export the model
        print('Export the onnx model ...')
        torch.onnx.export(model,
                          x,

...
class YOLOV4(object):
	...
	def get_engine(engine_path):
		...
		with open(engine_path, "rb") as f, trt.Runtime(trt.Logger(min_severity=trt.Logger.Severity.ERROR)) as runtime:
	...
	@staticmethod
	def trt_inference(context, bindings, inputs, outputs, stream, batch_size=1):
	...
	def _detect(self, list_of_imgs):
		...
		for batch in batches:
			with self.trt_engine.create_execution_context() as context:
				...
				trt_outputs = self.trt_inference(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream, batch_size=len(batch))
				features = trt_outputs[0].reshape(1, -1, 4 + len(self.class_names))

...
def yolo_forward_alternative(output, conf_thresh, num_classes, anchors, num_anchors, only_objectness=1,
    ...
    anchor_tensor = torch.tensor(anchor_array, device=device, dtype=output_dtype)
...
def yolo_forward(output, conf_thresh, num_classes, anchors, num_anchors, only_objectness=1,
    ...
    for i in range(num_anchors):
        ii = i * 2
        # Shape: [batch, 1, H, W]
        bx = bxy[:, ii] + torch.tensor(grid_x, device=device, dtype=output_dtype) # grid_x.to(device=device, dtype=torch.float32)
        # Shape: [batch, 1, H, W]
        by = bxy[:, ii + 1] + torch.tensor(grid_y, device=device, dtype=output_dtype) # grid_y.to(device=device, dtype=torch.float32)
        # Shape: [batch, 1, H, W]
